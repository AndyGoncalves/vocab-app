...
if not st.session_state.logged_in:
    ...
    if not vocab:
        ...
        for word, data in vocab.items():
            st.subheader(word)
            st.markdown(f"📖 **Definição**: {data['definition']}")
            st.markdown(f"🔤 **IPA**: {data['ipa']}")
            st.markdown(f"✏️ **Frase**: _{data['frase']}_")
            if data["audio_url"]:
                audio_path = f"users/{st.session_state.username}/audio/{word}.mp3"
                if os.path.exists(audio_path):
                    st.audio(audio_path)
                else:
                    st.warning("⚠️ Áudio não disponível.")